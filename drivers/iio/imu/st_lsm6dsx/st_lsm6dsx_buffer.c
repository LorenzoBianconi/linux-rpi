...
{
	u16 fifo_watermark = ~0, cur_watermark, sip = 0, fifo_len;
	...
	fifo_len = fifo_watermark * ST_LSM6DSX_SAMPLE_DEPTH;
		...
		while (acc_sip > 0 || gyro_sip > 0) {
			if (gyro_sip-- > 0) {
				...
				iio_push_to_buffers_with_timestamp(
					...
					iio_buff, gyro_sensor->ts);
				offset += ST_LSM6DSX_SAMPLE_SIZE;
				gyro_sensor->ts += gyro_sensor->delta_ts;
			}
			...
			if (acc_sip-- > 0) {
				...
				iio_push_to_buffers_with_timestamp(
					...
					iio_buff, acc_sensor->ts);
				offset += ST_LSM6DSX_SAMPLE_SIZE;
				acc_sensor->ts += acc_sensor->delta_ts;
			}
		}
	}
...
static irqreturn_t st_lsm6dsx_handler_irq(int irq, void *private)
{
	...
	delta_ts = (timestamp - hw->ts) * hw->sip;
	delta_ts = div_s64(delta_ts, hw->watermark);
	...
	for (i = 0; i < ST_LSM6DSX_ID_MAX; i++) {
		...
		if (sensor->sip > 0) {
			/*
			 * compute delta timestamp between two consecutive
			 * samples in order to estimate queueing time of data
			 * generated by the sensor
			 */
			sensor->ts = hw->ts;
			sensor->delta_ts = div_s64(delta_ts, sensor->sip);
		}
	}
	hw->ts = timestamp;
